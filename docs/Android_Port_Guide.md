...
### 第 5 步: Kotlin 原生播放与坐标映射验证 (进行中)
通过配置获取不同设备的屏幕比例，实现按键名如 'Q' 到屏幕 XY 的坐标映射算法（**已在 `TimelinePlayer.kt` 中初步搭好映射计算公式骨架，等待后续补充 1920x1080 基准坐标字典**）。在 Kotlin 端实现高精度执行调度器，验证单音、琶音与并发和弦在原神内的执行精准度。

### 附录: 《原神》“风物之诗琴”多分辨率坐标变换公式
基于观察得到的原神的 UI 缩放策略推断，对于任意目标分辨率，坐标映射公式如下：

1. **基准设定**: 基准分辨率为 `W_base = 1920`, `H_base = 1080`。
2. **计算缩放比 (Scale)**: 
   由于防止 UI 过小，采用宽和高中变化更大的比例作为整体防裁剪缩放比：
