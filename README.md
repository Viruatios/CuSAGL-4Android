...
12. bugfix1. 修复横屏游戏模式下悬浮窗因安全区不足而自动关闭的问题：悬浮窗位置不足时改为贴顶约束而不是退出服务，并取消状态栏/刘海对悬浮窗的上边界限制；同时缩小悬浮控制面板、移除面板内错误/日志显示，横屏宽屏时使用紧凑单行按钮和同一行左右状态显示；在开始演奏后，移除外层背景/阴影、将面板设为 50% 半透明。
    - 在此期间，确定了“开发”和“修复”的命名和规划区分：前者采用 `stepX`，版本号更新为 v `x.y+1.0`；后者采用 `bugfixX`，版本号更新为 v `x.y.z+1`。重大更新时，版本号更新为 v `x+1.0.0`。
13. bugfix2. 将“新建曲谱”从曲谱管理页内展开调整为独立页面，迁移原有手动创建表单并清空所有默认填充值；保存成功后返回曲谱管理页并刷新曲谱/播放准备状态，同时将 Android 版本更新为 `v1.0.1`。
14. 完成 Step9 关于页面与应用内更新检查：新增首页右上角三点入口和“关于”页面，展示版本号与仓库地址；通过 GitHub 最新正式 Release 与本地语义版本比较检查更新，发现新版本时下载 `apk-debug.apk` 到应用私有缓存并拉起系统安装器；补充 FileProvider、网络/安装权限、更新缓存清理策略和关于页逻辑单元测试，并落地 `CopilotDocs/step9/plan.md`。
